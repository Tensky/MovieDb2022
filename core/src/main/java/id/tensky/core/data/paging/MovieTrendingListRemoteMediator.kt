...
class MovieTrendingListRemoteMediator(
    ...
    override suspend fun load(
        ...
        return try {
            ...
            val networkResult = remoteSource.getMovieTrendingList(apiToken, loadKey)
            if (networkResult is ApiResponse.Success) {
                if (networkResult.data.isEmpty()) {
                    return MediatorResult.Success(endOfPaginationReached = true)
                }
                localSource.insertTrendingMovieList(networkResult.data.map {
                    it.toEntity()
                })
            }
